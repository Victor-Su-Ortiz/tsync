          ...
          while (tempDate < nextBusyStart) {
            ...
            if (preferredDays.includes(dayOfWeek)) {
              ...
              for (const range of timeRanges) {
                ...
                const effectiveStart = new Date(Math.max(rangeStart.getTime(), currentTime.getTime()));
                const effectiveEnd = new Date(Math.min(rangeEnd.getTime(), nextBusyStart.getTime()));
                
                // Check if this slot is large enough for the meeting
                const availableDuration = (effectiveEnd.getTime() - effectiveStart.getTime()) / (60 * 1000);
