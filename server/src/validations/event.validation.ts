...
export const eventValidation = {
  ...
  createEvent: Joi.object({
    ...
    recurrence: Joi.object({
      ...
      byDay: Joi.array().items(Joi.string().valid('MO', 'TU', 'WE', 'TH', 'FR', 'SA', 'SU')),
      byMonthDay: Joi.array().items(Joi.number().integer().min(1).max(31)),
  ...
  updateAttendeeStatus: Joi.object({
    status: Joi.string().valid('accepted', 'declined', 'pending').required().messages({
